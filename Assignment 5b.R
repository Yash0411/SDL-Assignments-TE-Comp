# Yash Nandkumar Morankar
# TECOB220  

# Funcions for perticular action
ds_mean <- function(ds) {
  cat("\nMean:-\n")
  print(sapply(ds[,1:4], mean, na.rm = T)) # find mean deviation for all columns
}

getmode <- function(v) {  # Function to calulate mode of each column
  uniqv <- unique(v)
...
ds_median <- function(ds) {
  cat("\nMedian:-\n")
  print(sapply(ds[,1:4], median, na.rm = T)) # find median deviation for all columns
}

ds_standard_deviation <- function(ds) {
  cat("\nStandard deviation:-\n")
  print(sapply(dataset[,1:4],sd)) # find standard deviation for all columns
}

ds_varaition <- function(ds) {
  cat("\nVariance:-\n")
  print(sapply(dataset[,1:4],var)) # find variance for all columns
}
...
ds_skewness <- function(ds) {
  ...
  library (moments)
  print(sapply(ds[,1:4],skewness)) # find skewness for all columns
}
...
ds_kurtosis <- function(ds) {
  ...
  #install.packages ("moments")
  library (moments)
  print(sapply(ds[,1:4],kurtosis)) # find kurtosis for all columns
}
